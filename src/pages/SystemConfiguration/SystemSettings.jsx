...
export default function SystemSettings() {
  ...
  const { defaultLanguage, setDefaultLanguage, supportedLanguages } =
    useLanguage();
  ...
  useEffect(() => {
    ...
    fetch(API_ENDPOINTS.SYSTEM_LANGUAGE, {
      ...
      .then(async (response) => {
        ...
        if (!response.ok)
          throw new Error(data.detail || t("systemSettings.loadError"));
        return data;
      })
      .then((data) => {
        if (!active) return;
        const language = isSupportedLanguage(data.default_language)
      ...
      .catch((requestError) => {
        if (active)
          setError(requestError.message || t("systemSettings.loadError"));
      })
      .finally(() => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const data = await response.json().catch(() => ({}));
      if (!response.ok)
        throw new Error(data.detail || t("systemSettings.saveError"));
  ...
  return (
    <div>
      <PageMeta
        title={`${t("systemSettings.title")} | VCC Plastics`}
        description={t("systemSettings.languageDescription")}
      />
            ...
            {isLoading ? (
              ...
              <Stack spacing={2}>
                <Typography variant="body2">
                  {t("systemSettings.currentDefault")}:{" "}
                  <strong>{t(`languages.${defaultLanguage}`)}</strong>
                </Typography>
                <FormControl size="small" fullWidth>
                  <InputLabel id="default-language-label">
                    {t("systemSettings.selectLabel")}
                  </InputLabel>
                  <Select
                    labelId="default-language-label"
                    value={selectedLanguage}
                    label={t("systemSettings.selectLabel")}
                    onChange={(event) =>
                ...
                <Alert severity="info">
                  {t("systemSettings.temporaryNote")}
                </Alert>
                <Box>
                  <Button
                    variant="contained"
                    ...
                    disabled={isSaving || selectedLanguage === defaultLanguage}
                    onClick={handleSave}
                    sx={{
                      bgcolor: "#005BAB",
