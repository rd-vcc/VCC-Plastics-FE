...
const navItems = [
  ...
  },
  {
    ...
    children: [
      ...
      {
        name: "Mold Installation History",
  ...
  },
  {
    ...
    children: [
      ...
      {
        name: "Machine & Equipment Maintenance",
      ...
      {
        name: "Production Tool Maintenance",
  ...
  },
  {
    ...
    icon: <PieChartIcon />,
    children: [
      { name: "Report Center", path: "/reports-analytics/report-center" },
    ],
  },
  {
    name: "Administration",
...
function containsActivePath(item, pathname) {
  ...
  return (
    item.children?.some((child) => containsActivePath(child, pathname)) ?? false
  );
}
...
function filterItemsByViewPermission(items) {
  return items.reduce((visibleItems, item) => {
    if (item.path) {
      ...
      if (!permissions || hasPermission(permissions.view))
...
const AppSidebar = () => {
  ...
  const visibleNavItems = useMemo(
    () => filterItemsByViewPermission(navItems),
    [],
  );
  const activeAncestorKeys = useMemo(
    () => collectActiveAncestorKeys(visibleNavItems, location.pathname),
  ...
  const renderItems = (items, depth = 0, parentKey = "root") => (
    <ul className={depth === 0 ? "flex flex-col gap-1.5" : "space-y-0.5"}>
            ...
            <li key={key}>
              ...
              >
                ...
                {showLabels && (
                  <>
                    {depth > 0 && (
                      <CircleDotIcon className="h-3.5 w-3.5 shrink-0 text-current" />
                    )}
                    <span className="min-w-0 flex-1 text-left">
                      {t(`navigation.${toTranslationSlug(item.name)}`, {
        ...
        return (
          <li key={key}>
            ...
            >
              {depth === 0 && item.icon && (
                <span
                  className={`menu-item-icon-size ${branchActive ? "menu-item-icon-active" : "menu-item-icon-inactive"}`}
                >
                  {item.icon}
                </span>
              )}
  ...
  return (
    <aside
      className={`fixed left-0 top-0 z-50 mt-16 flex h-screen flex-col border-r border-gray-800 bg-gray-900 px-4 text-gray-100 transition-all duration-300 ease-in-out lg:mt-0 ${
    ...
    >
      <div
        className={`flex py-5 ${!isExpanded && !isHovered ? "lg:justify-center" : "justify-start"}`}
      >
        <Link to="/">
          <img
            src="/images/logo/logo-dark.png"
            alt="Logo"
            width={showLabels ? 220 : 46}
            height={showLabels ? 60 : 46}
          />
        </Link>
      </div>
      ...
      <div className="flex flex-col overflow-y-auto pb-16 duration-300 ease-linear no-scrollbar">
        <nav className="mb-4">
          <h2
            className={`mb-2.5 flex text-[10px] uppercase leading-4 text-gray-400 ${!isExpanded && !isHovered ? "lg:justify-center" : "justify-start"}`}
          >
            {showLabels ? (
              t("common.menu")
            ) : (
